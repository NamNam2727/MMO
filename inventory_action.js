    ...
    if (!window.__invDOMEventsRegistered) {
        ...
        window.addEventListener('pointermove', (e) => {
            ...
            if (window.isDraggingItem) {
                ...
                if (window.invWindow && window.invWindow.style.display === 'flex') window.checkAutoScroll(e.clientY);
            }
        });
        ...
        window.addEventListener('pointerup', (e) => {
            ...
            if (window.dragState.active) {
                ...
                if (dropTarget) {
                    // =========================================================
                    // ★修正: ファジードロップ判定（ショップのどこに落としてもOKにする）
                    // =========================================================
                    ...
                    if (shopWin && window.shopState && window.shopState.isOpen && window.shopState.mode === 'sell') {
                        ...
                        if (targetSlotIdx !== -1) {
                            if (typeof window.promptShopSellCount === 'function') window.promptShopSellCount(window.dragState.item, targetSlotIdx);
                        } else {
        ...
        document.getElementById('btnUseEquip').addEventListener('pointerdown', (e) => {
            ...
            if (btn.dataset.isChip === "true") {
                if(typeof window.openSkillCreateFromItem === 'function') window.openSkillCreateFromItem(item);
                itemDetail.style.display = 'none'; return;
            }
            if (btn.dataset.isSkill === "true") {
                if(typeof window.executeSkill === 'function') window.executeSkill(item);
                itemDetail.style.display = 'none'; return;
            }
            ...
            if (item.type === 'equip') {
                ...
                window.itemCooldowns[item.id] = 2.0; window.itemMaxCooldowns[item.id] = 2.0;
                item.count--;
                if (item.count <= 0) {
                    window.player.inventory[tabId].items.splice(window.selectedItemIndex, 1);
                    itemDetail.style.display = 'none'; window.selectedItemIndex = -1;
                } else {
                    if (typeof window.showItemDetail === 'function') {
                        const slots = document.querySelectorAll('.inv-slot');
                        if(slots[window.selectedItemIndex]) window.showItemDetail(item, slots[window.selectedItemIndex]);
                    }
                }
            }
            if (typeof window.renderInventory === 'function') window.renderInventory();
...
window.checkAutoScroll = function(y) {
    ...
    const rect = invContent.getBoundingClientRect(); const threshold = 35; 
    if (y > rect.top && y < rect.top + threshold) window.startAutoScroll(-6); 
    else if (y < rect.bottom && y > rect.bottom - threshold) window.startAutoScroll(6);  
    else window.stopAutoScroll();
};
window.startAutoScroll = function(dy) {
    if (!window.scrollInterval) window.scrollInterval = setInterval(() => { const invContent = document.getElementById('invContent'); if (invContent) invContent.scrollTop += dy; }, 20);
};
window.stopAutoScroll = function() {
    if (window.scrollInterval) { clearInterval(window.scrollInterval); window.scrollInterval = null; }
};
...
window.addItemToInventory = function(item) {
    ...
    if (!added) {
        if (typeof window.addLog === 'function') window.addLog(`<span class='color-sys'>インベントリがいっぱいで ${item.name} を拾えなかった。</span>`, 'sys');
